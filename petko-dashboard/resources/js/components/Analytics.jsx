...
export default function Analytics() {
  ...
  // Fix: filter daily by full date prefix, not sliced value
  const dailyData = useMemo(() => {
    ...
    return entries
      ...
      .map(e => ({ date: e.date.slice(8), amount: e.amount })); // show day only when filtered
  }, [data, month]);
  ...
  const totals = useMemo(() => {
    ...
    if (!filteredMonthly.length) return zero;
  ...
  // Fix: use per-month category/product data when a month is selected
  const categoryData = useMemo(() => {
    if (!data) return [];
    if (month === 'all') {
      return Object.entries(data.categories).map(([name, value]) => ({ name, value }));
    }
    const m = data.monthly.find(m => m.month === month);
    if (!m) return [];
    return Object.entries(m.categories).map(([name, value]) => ({ name, value }));
  }, [data, month]);
  ...
  const topProducts = useMemo(() => {
    ...
    const m = data.monthly.find(m => m.month === month);
    if (!m) return [];
    return Object.entries(m.top_products).map(([name, value]) => ({ name, value }));
  }, [data, month]);
    ...
    <>
      ...
      <div className="charts-grid">
        <div className="card chart-full">
          <h3>Daily Sales {month !== 'all' && `— ${new Date(month + '-02').toLocaleString('default', { month: 'long', year: 'numeric' })}`}</h3>
          ...
          <table>
            ...
            <tbody>
              {filteredMonthly.map(m => (
                <tr key={m.month}>
                  <td>{new Date(m.month + '-02').toLocaleString('default', { month: 'long', year: 'numeric' })}</td>
